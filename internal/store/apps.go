...
const appCols = `id, name, secret, public, provider_id, redirect_uris, scopes, enabled, created_at, updated_at`
...
func scanApp(row interface{ Scan(...any) error }) (*models.App, error) {
	...
	err := row.Scan(&a.ID, &a.Name, &a.Secret, &a.Public, &providerID,
		&uris, &scopes, &a.Enabled, &created, &updated)
	if err != nil {
...
func (s *Store) CreateApp(a *models.App) error {
	...
	_, err := s.db.Exec(`
		INSERT INTO applications (id, name, secret, public, provider_id, redirect_uris, scopes, enabled, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		a.ID, a.Name, a.Secret, a.Public, nullable(a.ProviderID),
		urisToJSON(a.RedirectURIs), joinScopes(a.Scopes), a.Enabled, ts, ts)
	return err
}
...
func (s *Store) UpdateApp(a *models.App) error {
	res, err := s.db.Exec(`
		UPDATE applications
		SET name = ?, secret = ?, public = ?, provider_id = ?, redirect_uris = ?, scopes = ?, enabled = ?, updated_at = ?
		WHERE id = ?`,
		a.Name, a.Secret, a.Public, nullable(a.ProviderID),
		urisToJSON(a.RedirectURIs), joinScopes(a.Scopes), a.Enabled, now(), a.ID)
	if err != nil {
		return err
	}
